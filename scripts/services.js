...
//---------------------NEW CODE JUNE 13 2017 ADDING DEMO OBJECT TO DEMO------------- PAGE-------------------------------------------------------------------------------
            var demo=[
                        ...
                        },
                {
                          ...
                          info:'info',
                          description:'Scope is an object that refers to app.model. ***At core of angular 2-way data binding, scope is the glue between view and controller. The controller can set $rootScope by being assigned to the highest order in the DOM tree. Angular $scope has a direct relation with the html DOM tree. In other words $rootScope of app scope tree has relationship with the DOM tree, you have support for nested scopes. The child $scope has access to parent scope, something similar to js prototypes. In controllers $scope.variable replaces this.variable when we have to define them in angular controllers. In the examples below we create an angular app by assigning it to variable app and we also assign this app to our body of the page. It is important when we create an angular app to pass it to a variable. Otherwise we won\'t initialize our angular app correctly. You can test that yourself by trying to paste example 2 before example one and example 2 controller won\'t work. Inside our app we create 2 controllers MyCtrl and GunController and on the html we assign the appropriate divs to those controllers. The first example is a simple illustration of how to create a simple controller. It is an example of angulars direct 1 way data binding from the javascript controller to the html angular expression.  In the second example we are using an advanced technique of javascript inline array notation where we create a controller we can chain onto our app module. In the second example we are also illustrating 2 way data binding by creating variable model and assigning it to ng-model directive inside an input element and right after displaying it via an angular expression. So the ng-model directive model binding is 1/2 data bindings and angular expression model binding is 2/2. Note: this example has a working angular CDN for JSBIN version 1.01 compatible with 1.63 which is the version I\'m used to.   '
                        },
