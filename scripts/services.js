...
//---------------------NEW CODE JUNE 13 2017 ADDING DEMO OBJECT TO DEMO------------- PAGE-------------------------------------------------------------------------------
            var demo=[
                        ...
                        },
                {
                          ...
                          link:'',
                          info:'info',
                          description:'A single page application at its core is a hyperlink that specifies a link to a searchable or indexed piece of web content. example: http://www.conFusion.food/index.html/menu/0 the last part menu/0 becomes interchangeable in a single page application(SPA).SPAs hijack the hashtag approach that is used by hyperlinks. Through the changing portion of the URL, we can load up different views creating the single page application. The role of the server in a single page app is to serve up data using REST API, it also supplies static HTML pages, angular templates, and adds/stores resources. The clients role is to render the view and manage templating and routing. A single page app is a web app or site that fits in a single page, there is no need to reload the page for it to work properly. UX is like a desktop or native app. Most resources returned with a single page load.  Redraw parts of the page when needed without requesting the whole page again. In a SPAs the clients role is to fetch data and render data. Angular provides support for single page applications through routing. "To better understand how SPAs are orchestrated in angular lets examine a concept called Deep Linking. If you look at a page all resources are identified by using a URI. Deep linking allows you to specify a hyperlink that specifies a link to specific searchable or index piece of web content. Resource like an image, link inside of a page. Link to some other resource that is included in your web page. All of these can be identified using a deep link. Typically intrapage references are using the hashtag followed by a link. What single page apps does is hijack the hashtag approach. ex. http://www.conFusion.food/index.html.#/menu/0 Anything following the hash doesnt cause any page reload because the browser thinks the resource is within the page itself ***If you can keep changing the info that follows the hash, then you are easily able to load different *views* w/o causing a page reload. "The route in the hash decides which way to move, any change to the hash portion of the URL does not require a page reload." In single page applications and angular we have an important service called $location that gets a hold of the current URL in browser address bar and changes the hash portion of the url as well. Corresponding to the hash portion change we can display different views. $location allows to both get and set the hash portion of the url. "If you can keep swapping the views without reloading the page you have a single page application in action. To help us do that in angular we have the route module or the ui view module. Routing in SPAs is mapping the path portion of a URL to a handler for that particular route. A route in SPA is the hash # symbol and stuff that follows is the path, we can use angular ngRoute or UIrouter to map/route that portion to a handler that is usually  routeProvider or stateProvider angular service and under these handler services we define where the route is comming from what #portion and handle those routes with html templates. So to summarize we have single page applications and they use the hash portion of the url to switch between different views without reloading the page. These hash portions that are changing and related to views are done by ngRoute or UIRouter in angular we can define in our angular app a routeProvider or a stateProvider that when matched with particular hash portion will display a corresponding angular template via ng-views ur ui-views. To help us swap in between use we use the route module. '
                        },
                {
                          _id:54,
                          name:'ngRoute',
                          image: '',
                          category: 'Angular ',
                          link:'http://jsbin.com/qucapoduga/edit?html,js,output',
                          info:'info',
                          description:'When we have single page applications we have hash portions of the url that we map to different views. If we can swap between views without reloading the page we have a SPA. To help us achieve this function using the hash urls we can use the angular ngRoute module. ngRoute "manages the interaction between the $location service and the rendered view. It also uses the location service to keep a watch on the url and also can update the hash portion of the URL if required." Little tricky is this $location service. The $location service is behind the scenes mostly but is responsible for managing the hash portion of our urls. The $location service exposes the current url in the browser address bar, we cab use it to extract the hash portion of the url. ngRoute is not part of the angular core we have to install it or use a CDN and then also we have to do dependency injection into our angular module for example angular.module(\'confusionApp\',[\'ngRoute\']). ngRoute is like you are tearing one part of the page and putting in a new part right in there. The ngResource comes with $routeProvider that helps to map routes to handlers. It is important to note that the $routeProvider needs to be configured using .config. In our configurations we define our handler objects. Handlers in ngRoute are objects that define template URL and controller. So $routeProvider of ngRoute allows use to map routes to templates. The $routeProvider provides us with a the .when() function. .when() takes 2 parameters, first the url, then the html template. If the hash portion of the URL matches, render the corresponding template view. We also specify the controller corresponding to that view. With .when() function and its 2 parameters url and template we let $routeProvider know what to render and when there is a request to that particular hash url. The routeProvider also takes in a default route via .otherwise() function so if a request to the hash portion is made and it doesn\'t match up to any routeProvider .when()s then treat this hash as the defualt homepage route aka .otherwise(\'/\'). ngRoute module also has support for $routeParams. When we .config[ged] our $routeProvider we specified one of the hash portion urls as /menu/:id. This :id becomes property of the $routeParams so we can go use that selected param id in our controller code. Whats super interesting when we set up our hyperlinks in templates we first need to use ng-href so our $location service can track the ng-href url portion. When you specify the ng-href you can do something like ng-href="/menu/{{dish._id}}" where dish._id is individual dish from the total dishes object. Usually you partition the entire object array using for each loop, so you can capture each individual id. With this ng-href and dynamic links we pass those url links through our $location service to our ngRoute module that takes care of the hash urls via its $routeProvider. So the $routeProvider takes these urls that where given by ng-href, and begins deciphering them so it can know what do do with the templates they represent. By reading these ng-href hash urls with the help of $routeParams $routeProvider can identify the id we mean to provide as a parameter. If done right the /:id parameters will become properties of the $routeParams. So if the first object is chosen in our object array list it will represent _id=0 so ng-href will pass 0 to $routeProvider which will pass 0 to $routeParams which will pass 0 to controller which will call the resource service to dynamically fetch just the resource with index 0.   In order to process these parameters we can do that in our controllers (remember controllers are already linked in our .config so now we have to include some config code in our controllers to make use of those url parameters that are generally the task of ngRoute\'s $routeProvider but it can\'t do everything so we give the duty of handling parameters to $routeParams and our controller code can inject $routeParams and make use of those parameters that represent those route hash urls. We will next look at UI Router, that helps us map routes to states, that can have multiple interchangeable html templates in each state. Usually we have header body and footer sections defined in each state. In ngRoute we don\'t have states just standard views with only one template that would have all three components into one header body footer.  '
                        },
                {
                          _id:55,
                          name:'UI-Router',
                          image: 'images/5x.PNG',
                          category: 'Angular',
                          link:'http://jsbin.com/cofacegona/edit?html,js,output',
                          info:'info',
                          description:'***"State Machine Approach in designing single page applications: Instead of routes to track different views, you specify states, that move from one state to another. Router as a state machine." UI-Router is another external angular module that replaces the role of ngRoute in single page applications by taking over the display of views through states. While ngRoute application views where tied with URLs, UI-Router uses states that render views. When we .config or ui-router after we do dependency injection in our main angular module we first pass in an annon function to .config and in that anon function we pass 2 parameters $stateProvider and $urlRouterProvider. $stateprovider is very similar to $routeProvider in ngRoute. The core of the $stateProvider service is the .states() method. .states takes a state name as a first parameter, for example \'app\' and then for second parameter we pass in a route to our app state, if you want to route to homepage you can specify url:\'/\', then the third parameter in our .state definition is the views object, that we express ass views:{\'header\':{templateUrl"\'views/header.html\'}}, \'content\': {}, \'footer\':{} }. This views object is important because it is our handler to our route url requests in browser and it also responds to inapp requests for example using ui-sref directive to specify the name of the app. Notice when you use ui-sref you are not specifying the url of the state but the name of the state itself.  Then in the end when we are done chaining in all our states we can utilize the $urlRouterProvider.otherwise(\'/\'); to declare a default route. Ok I sometimes forget the  meaning of all of this especially what does it mean by the word "route". Importantly UI-Router is an expansion of ngRoute and Single Page Applications. Single page applications are nothing more than hashtag interdocumented links. So when we mean by "route" in UI-router we are talking about the hashtag and on portion of the actual url. This information is the same for ngRoute single page applications, both modules use the hashtag portion and on of the url yo manipulate between views. So just like in ngRoute when we config our $stateProvider we are configuring hashtag urls because we are dealing with dynamic urls exploted for single page applications and UI-Router particularly exploits these because we can combine multiple views per single URL and dynamically change up other parts of page through the hashurls. Since hashurls can be infinite and now we can nest multiple views per url/state this creates an endless possibility of data manipulation and display for the front end user. After defining our states using the $stateProvider service of the UI-Router angular module we can also utilize the $stateParams service. $stateParams is just like $routeParams in terms that you can capture hashtag url parameters by separating them with : like home/:this creating this variable that gets binding to the $routeParam Object. On the front end side we can use dynamic data binding to add a particular id variable to a ui-sref directive so that way that variable if passed in as a right parameter will be captured in our state definition. So we can use databinding to tag a variable that we pass in as a parameter in the url and the our $stateProvider and $stateParams will capture that variable, usually in form of an index to produce pretty urls, and then pass it to some angular controller, so the point of all of this is that when a front end user clicks on an ui-sref link, through the angular router system if we binded a variable to the link based on what link he clicked we can pass that intel through thr $stateParams and store it into some angular controller. heres an interesting look on how an a ui-sref looks like : a ui-sref = "app.dishDetails({id:dish_id})" notice how we are saying that that variable id is gonna be the propagaded dishes id at the time of our ng-repeat dish in dishes loop. In conclusion UI-Router module and the $stateProvider service in use with .state() method allow us to define angular states, that come with a name are are mapped to an url. Then they are also associated with stacked multiples of views, in top to bottom order. In a way states become like html divs. Also I heard that when you utilize UI-routers and the state approach you are giving complete control of rendering of html to angular and away from the browser. This allows more of an app user experience and not a traditional website look and feel.   '
                        },
                {
                          _id:56,
                        ...
                        },
                {
                          ...
                          image: 'images/55.PNG',
                          category: 'HTML DOM',
                          link:'http://jsbin.com/dimudiqone/edit?html,js,console,output',
                          info:'info',
                          description:' This code uses pure javascript to make an onclick event loop that registers all button clicks you can filter by button type or class or whatever initially and then you can also filter by the button value. In the example i have 3 buttons with values 50, Null, and 25 and they all generate a 1 console log because they all button types but value 50 also generates additional console log because there is a condition for button value 50. This can be useful in your case because you can use ng-repeat to generate buttons and have their values be something like: value="{{button.id}}-button-value" so you can capture each button click with button.id in your controller'
                        },
...
var samples=[            {
                          ...
                          url2: 'https://lasertag.cc/gun.html',
                          name2: 'Lasergun Mode'
                        },
                         {
                          _id:1,
